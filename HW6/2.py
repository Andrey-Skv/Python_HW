# 2. Для чисел в пределах от 20 до N найти числа, кратные 20 или 21. Use comprehension.
# in
# 100

# out
# [20, 21, 40, 42, 60, 63, 80, 84, 100]

# in
# 424

# out
# [20, 21, 40, 42, 60, 63, 80, 84, 100, 105, 120, 126, 140, 147, 160, 168, 180, 189, 200, 210, 220, 231, 240, 252, 260, 273, 280, 294, 300, 315, 320, 336, 340, 357, 360, 378, 380, 399, 400, 420]
num = int(input('Введите число: '))

def multiple_of_twenty_or_twentyone(N):
    return [num for num in range(20, N+1) if num % 20 == 0 or num % 21 == 0]
 
print(multiple_of_twenty_or_twentyone(num))

# def multiple_of_twenty_or_twentyone(m):
#     lst = []
#     for num in range(20,m+1):
#         if num %20==0 or num%21==0:
#             lst.append(num) 
#     return lst

# print(multiple_of_twenty_or_twentyone(num))
